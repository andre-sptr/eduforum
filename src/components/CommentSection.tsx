...
import { Send, MoreHorizontal, Trash2, Image as ImageIcon, X } from "lucide-react";
...
export const CommentSection = ({ postId, currentUserName, currentUserInitials, currentUserId }: CommentSectionProps) => {
  ...
  const { data: comments = [], isLoading } = useQuery<Comment[]>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("comments").select("id, user_id, content, image_url, created_at, profiles(name, avatar_text)").eq("post_id", postId).order("created_at", { ascending: true });
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  {c.content && <p className="text-sm mt-1 whitespace-pre-wrap break-all">{c.content}</p>}
