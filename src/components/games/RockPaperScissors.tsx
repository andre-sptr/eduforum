...
const RockPaperScissors = ({ currentUserId, onScoreSubmit }: RPSProps) => {
  ...
  const determineWinner = (my: Choice, opp: Choice): "win" | "lose" | "draw" => {
      ...
      .on(
        ...
        async (payload) => {
          ...
          if (updated.status === "active" && updated.game_state) {
            const state = updated.game_state as unknown as GameState;
            const isPlayer1 = currentUserId === updated.player1_id;

            const p1c = state.player1Choice;
            const p2c = state.player2Choice;

            setRounds(state.rounds || 0);
            ...
            if (p1c && p2c) {
              setMyChoice(isPlayer1 ? p1c : p2c);
              setOpponentChoice(isPlayer1 ? p2c : p1c);
              const r = determineWinner(isPlayer1 ? p1c : p2c, isPlayer1 ? p2c : p1c);
              setResult(r);

              if (isPlayer1) {
                const oldRounds = state.rounds;
                const next: GameState = { ...state };
                const winnerRel = determineWinner(p1c, p2c);
                if (winnerRel === "win") next.player1Score += 1;
                else if (winnerRel === "lose") next.player2Score += 1;
                next.rounds = oldRounds + 1;
                ...
                const { error } = await supabase
                  ...
                  .update({
                    ...
                    status: next.rounds >= 3 ? "finished" : "active",
                    winner_id:
                      next.rounds >= 3
                        ? next.player1Score > next.player2Score
                          ? updated.player1_id
                          : next.player2Score > next.player1Score
                          ? updated.player2_id
                          : null
                        : null,
                  })
  ...
  const findMatch = async () => {
    ...
    try {
      const { error: queueError } = await supabase
        ...
        .upsert({ user_id: currentUserId, game_type: "rps" }, { onConflict: "user_id,game_type" });
      ...
      if (waiting) {
        ...
        const { data: updated, error } = await supabase
          ...
          .update({
            ...
            status: "active",
            game_state: initState as unknown as Json,
          })
          .eq("id", waiting.id)
          .is("player2_id", null)
          .eq("status", "waiting")
        ...
        if (error || !updated) {
          ...
          toast.message("Sesi barusan sudah diambil, mencoba lagi...");
          return findMatch();
        }
      ...
      const { data: newSession, error: sessionError } = await supabase
        .from("game_sessions")
        .insert({ game_type: "rps", player1_id: currentUserId, status: "waiting" })
        .select()
        .single();
      ...
      const ch = supabase
        .channel(`matchmaking:rps:${newSession.id}`)
        .on(
          "postgres_changes",
  ...
  const makeChoice = async (choice: Choice) => {
    ...
    const next: GameState = {
      ...state,
      player1Choice: isPlayer1 ? choice : state.player1Choice,
      player2Choice: !isPlayer1 ? choice : state.player2Choice,
    };

    const { error } = await supabase
      .from("game_sessions")
      .update({ game_state: next as unknown as Json })
  ...
  const cancelSearch = async () => {
    try {
      ...
      await supabase.from("matchmaking_queue").delete().eq("user_id", currentUserId).eq("game_type", "rps");
    } finally {
      if (mmChannelRef.current) supabase.removeChannel(mmChannelRef.current);
      mmChannelRef.current = null;
      setGameSession(null);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between text-sm">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {loadingOpponent
                ? "Memuat…"
                : opponent?.full_name ?? "Tidak diketahui"}
            </p>
          </div>
        </div>
        ...
        >
          {myChoice ? (opponentChoice ? "Menentukan hasil…" : "Menunggu lawan…") : "Pilih tangan"}
        </span>
      </div>
      ...
      {myChoice && opponentChoice ? (
        <div className="flex justify-around items-center p-4">
          <div className="text-center">
            ...
            {myChoice === "scissors" && <ScissorsIcon className="h-12 w-12 text-primary" />}
          </div>
          <span className="text-2xl">VS</span>
          <div className="text-center">
            <p className="text-xs text-muted-foreground mb-0.5">Lawan</p>
            <p className="text-[11px] text-muted-foreground">
              {loadingOpponent ? "Memuat…" : opponent?.full_name ?? "Tidak diketahui"}
            </p>
            {opponentChoice === "rock" && <Hand className="h-12 w-12 text-accent" />}
            {opponentChoice === "paper" && <FileText className="h-12 w-12 text-accent" />}
            {opponentChoice === "scissors" && <ScissorsIcon className="h-12 w-12 text-accent" />}
          </div>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-3 gap-2">
            <Button onClick={() => makeChoice("rock")} disabled={!!myChoice} variant="outline" className="h-20 flex flex-col gap-1">
              <Hand className="h-6 w-6" />
              <span className="text-xs">Batu</span>
            </Button>
            <Button onClick={() => makeChoice("paper")} disabled={!!myChoice} variant="outline" className="h-20 flex flex-col gap-1">
              <FileText className="h-6 w-6" />
              <span className="text-xs">Kertas</span>
            </Button>
            <Button onClick={() => makeChoice("scissors")} disabled={!!myChoice} variant="outline" className="h-20 flex flex-col gap-1">
      ...
      {gameSession.status === "finished" && (
        <div className="text-center p-3 bg-accent/10 rounded-lg">
          ...
          <p className="font-semibold">
            {gameSession.winner_id === currentUserId ? "Kamu Menang Game!" : gameSession.winner_id ? "Kamu Kalah Game" : "Seri!"}
          </p>
        </div>
      )}
    </div>
