...
export default function ColorMatch({onScoreSubmit}:ColorMatchProps){
  ...
  const [wordColor,setWordColor]=useState(""); const [displayColor,setDisplayColor]=useState("");
  const [score,setScore]=useState(0); const [timeLeft,setTimeLeft]=useState(30); const [streak,setStreak]=useState(0);

  useEffect(()=>{ let t:any;
    if(gameState==="playing"&&timeLeft>0){ t=setInterval(()=>{ setTimeLeft(p=>{ if(p<=1){ finishGame(); return 0;} return p-1;}); },1000); }
    return()=>clearInterval(t);
  },[gameState,timeLeft]);

  const initializeGame=()=>{ setScore(0); setTimeLeft(30); setStreak(0); setGameState("playing"); nextRound(); };
  const nextRound=()=>{ const w=COLORS[Math.floor(Math.random()*COLORS.length)]; const d=COLORS[Math.floor(Math.random()*COLORS.length)]; setWordColor(w.name); setDisplayColor(d.text); };
  const handleAnswer=(isMatch:boolean)=>{ const actualMatch=wordColor===COLORS.find(c=>c.text===displayColor)?.name;
    if((isMatch&&actualMatch)||(!isMatch&&!actualMatch)){ const pts=10+(streak*2); setScore(v=>v+pts); setStreak(v=>v+1);} else setStreak(0);
    nextRound();
  };
  ...
  if(gameState==="idle") return (
    <div className="grid place-items-center gap-4 py-10 text-center">
      <p className="text-muted-foreground">Cocokkan kata dengan warna dalam 30 detik!</p>
      <Button onClick={initializeGame} size="lg" className="rounded-2xl bg-accent text-accent-foreground hover:bg-accent/90">
        <Play className="mr-2 h-4 w-4"/>Mulai Game
      </Button>
    </div>
  );
  ...
  if(gameState==="finished") return (
    <div className="grid place-items-center gap-4 py-10 text-center">
      <div>
        <div className="mb-1 text-4xl font-extrabold tracking-tight text-accent">{score}</div>
  ...
  return (
    <div className="space-y-5">
      <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
        <div className="h-full bg-accent transition-[width] duration-1000" style={{width:`${(timeLeft/30)*100}%`}}/>
      </div>
