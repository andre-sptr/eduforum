...
const scrambleWord = (word: string): string => {
  let s = fisherYates(word.split("")).join("");
  if (s === word) s = scrambleWord(word);
  return s;
};
...
const isGameState = (v: unknown): v is GameState => {
  const s = v as Partial<GameState> | null;
  return !!s
    && typeof s.round === "number"
    && typeof s.player1Score === "number"
    && typeof s.player2Score === "number"
    && typeof s.currentWord === "string"
    && typeof s.scrambledWord === "string"
...
const WordScramble = ({ currentUserId, onScoreSubmit }: WordScrambleProps) => {
  ...
  const mmChannelRef = useRef<any>(null);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (updated.status === "active" && isGameState(updated.game_state)) {
  ...
  useEffect(() => {
    if (gameSession?.status !== "active" || !isGameState(gameSession?.game_state)) return;
    const s = gameSession.game_state;
    const isP1 = currentUserId === gameSession.player1_id;
    setMyScore(isP1 ? s.player1Score : s.player2Score);
    setOpponentScore(isP1 ? s.player2Score : s.player1Score);
    setRound(s.round);
    setScrambledWord(s.scrambledWord);
    setHint(s.hint);
    setAnswer("");
  }, [gameSession, currentUserId]);

  const scrambledFromState = isGameState(gameSession?.game_state)
    ? gameSession!.game_state.scrambledWord
    : "";

  useEffect(() => {
    const trigger = scrambledFromState || scrambledWord;
    if (gameSession?.status !== "active" || !trigger) return;
    ...
    timerRef.current = setInterval(() => {
      ...
      if (left === 0) {
        if (timerRef.current) clearInterval(timerRef.current);
        if (gameSession?.player1_id === currentUserId) handleTimeUp();
      }
    }, 250);
    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
    };
  }, [gameSession?.id, gameSession?.status, scrambledFromState, scrambledWord, currentUserId]);
  ...
  const findMatch = async () => {
    ...
    try {
      const { error: queueError } = await supabase
        ...
        .upsert({ user_id: currentUserId, game_type: "wordscramble" }, { onConflict: "user_id,game_type" });
      ...
      if (waitingSessions) {
        ...
        const nextState: GameState = {
          ...
          scrambledWord: scrambleWord(wordObj.word),
          hint: wordObj.hint
        };
        ...
        if (updateError || !updated) {
          ...
          toast.message("Sesi sudah diambil, mencoba lagi...");
          return findMatch();
        }
        ...
        await supabase
          ...
          .delete()
          .eq("game_type", "wordscramble")
          .in("user_id", [currentUserId, waitingSessions.player1_id]);
        ...
        const { data: newSession, error: sessionError } = await supabase
          .from("game_sessions")
          .insert({ game_type: "wordscramble", player1_id: currentUserId, status: "waiting" })
          .select()
          .single();
        ...
        const ch = supabase
          .channel(`matchmaking:wordscramble:${newSession.id}`)
          .on(
            "postgres_changes",
  ...
  const advanceRound = async (currentState: GameState) => {
    ...
    if (currentState.round >= 3) {
      ...
      await supabase
        .from("game_sessions")
        .update({ status: "finished", winner_id: finalWinner, game_state: currentState as unknown as Json })
      ...
      await supabase
        .from("game_sessions")
        .update({ game_state: nextState as unknown as Json })
        .eq("id", gameSession.id);
    }
  };

  const submitAnswer = async () => {
    if (!answer.trim() || !gameSession || submitting) return;
    const state = isGameState(gameSession.game_state) ? gameSession.game_state : null;
    if (!state) return;
    if (answer.trim().toUpperCase() !== state.currentWord) {
    ...
    try {
      ...
      const key: keyof Pick<GameState, "player1Score" | "player2Score"> = isPlayer1 ? "player1Score" : "player2Score";
      const base = state as GameState;
      const updatedState: GameState = { ...base, [key]: base[key] + 10 };
      await advanceRound(updatedState);
    } finally {
      setSubmitting(false);
      setAnswer("");
    }
  ...
  const handleTimeUp = async () => {
    ...
    const state = isGameState(gameSession.game_state) ? gameSession.game_state : null;
    if (!state) return;
    toast.info(`Waktu habis! Jawaban: ${state.currentWord}`);
  ...
  const cancelSearch = async () => {
    try {
      ...
      await supabase.from("matchmaking_queue").delete().eq("user_id", currentUserId).eq("game_type", "wordscramble");
    } finally {
      if (mmChannelRef.current) supabase.removeChannel(mmChannelRef.current);
      mmChannelRef.current = null;
      setGameSession(null);
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {loadingOpponent ? "Memuat…" : opponent?.full_name ?? (!gameSession?.player2_id ? "Menunggu…" : "Tidak diketahui")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 bg-muted rounded-lg space-y-2">
        ...
        <p className="text-2xl font-bold text-center tracking-widest">
          {stateForView?.scrambledWord ?? scrambledWord}
        </p>
        {timeLeft === 0 && (
          <p className="text-xs text-destructive text-center font-semibold">
            Jawaban: {stateForView?.currentWord}
          </p>
        )}
      </div>
