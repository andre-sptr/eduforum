...
const profileSchema = z.object({
  ...
  avatar_text: z.string()
    .length(2, "Inisial harus terdiri dari 2 karakter"), 
  ...
  useEffect(() => {
    if (currentProfile) {
      reset({
        ...
        avatar_text: currentProfile.avatar_text || '',
    ...
    mutationFn: async (formData: ProfileFormData) => {
      ...
      const { error } = await supabase
        ...
        .update({
            name: formData.name,
            bio: formData.bio,
            avatar_text: formData.avatar_text
        })
        .eq('id', user.id);
