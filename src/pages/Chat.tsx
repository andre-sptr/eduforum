...
const Chat = () => {
  ...
  return (
    <div className="space-y-4 h-[505px]">
      <header className="border-b border-border bg-card shadow-sm rounded-t-2xl z-20 sticky top-0">
        <div className="flex items-center gap-4 px-6 py-4">
          <Button variant="ghost" size="icon" onClick={()=>navigate("/messages")} className="rounded-xl hover:bg-muted transition-colors"><ArrowLeft className="h-5 w-5"/></Button>
          <div className="flex-1">
            <h1 className="text-xl font-bold text-foreground">{conversation?.name||"Chat"}</h1>
      ...
      <Card ref={cardRef} className="flex-1 border-border bg-card shadow-xl overflow-hidden flex flex-col h-full rounded-b-2xl rounded-t-none mt-0 relative">
                    ...
                    <div className={`max-w-[75%] ${mine?"items-end text-right":"items-start"} flex flex-col`}>
                      ...
                      {editing?(
                        <div className={`flex ${mine?"flex-row-reverse":""} items-center gap-2 w-full`}>
                          <Input value={editContent} onChange={e=>setEditContent(e.target.value)} className="h-10 bg-muted/50 border-muted rounded-xl" autoFocus onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); handleEditMessage(m.id); } if(e.key==="Escape") cancelEdit(); }}/>
                          ...
                          <Button size="sm" variant="ghost" onClick={cancelEdit} className="rounded-lg hover:bg-muted">Batal</Button>
                        </div>
                      ):(
                        <div className={`group relative inline-flex flex-col rounded-2xl px-5 py-3 shadow-sm border transition-all ${mine?"bg-primary text-primary-foreground border-primary rounded-tr-sm":"bg-muted/30 text-foreground border-border rounded-tl-sm hover:bg-muted/50"}`}>
          ...
          <form onSubmit={handleSendMessage} className="border-t border-border p-4 bg-card">
            <div className="flex gap-3 max-w-4xl mx-auto items-end">
              {conversation?.type==="direct"?(
                <Input value={newMessage} onChange={e=>setNewMessage(e.target.value)} placeholder="Ketik pesan..." className="flex-1 rounded-2xl bg-muted/50 border-muted focus:bg-background transition-all h-12 px-4 shadow-sm" disabled={sending}
                  onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); handleSendMessage(e as any); } }}/>
              ):(
                <MentionInput value={newMessage} onChange={setNewMessage} placeholder="Ketik pesan (@user)..." className="flex-1 rounded-2xl bg-muted/50 border-muted focus:bg-background transition-all min-h-[48px] px-4 py-3 shadow-sm resize-none" disabled={sending}
